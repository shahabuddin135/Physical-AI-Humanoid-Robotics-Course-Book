...
export default function DocPageActions({ content, pagePath }: DocPageActionsProps) {
  ...
  // Load user's saved edit for this page
  useEffect(() => {
    if (user && token && pagePath) {
      fetch(`${API_URL}/user/edits/${encodeURIComponent(pagePath)}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
        .then(res => res.json())
        .then(data => {
          if (data.content) {
            setUserEdit(data.content);
          }
        })
        .catch(() => {});
    }
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/user/edits`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
        ...
        <div className={styles.editContainer}>
          ...
          <div className={styles.editActions}>
            <button
              ...
              disabled={isSaving}
            >
              {isSaving ? <span className={styles.spinner} /> : 'Save Notes'}
            </button>
          </div>
        </div>
      )}
